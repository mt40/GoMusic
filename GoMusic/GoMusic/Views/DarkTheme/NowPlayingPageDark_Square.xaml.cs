...
{
    public partial class NowPlayingPageDark_Square : PhoneApplicationPage
    {
        public bool isTrayVolumeOpened = false;
        public DispatcherTimer SongTimer;

        public NowPlayingPageDark_Square()
        {
            InitializeComponent();
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            TextBlockSongInfo.DataContext = musicAlbum.DataContext = grid_Tray.DataContext = null;
            TextBlockSongInfo.DataContext = media_sv.CurrentSong;
            musicAlbum.DataContext = al;
        ...
        private void StackPanelRepeat_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            ...
            if (MediaPlayer.IsRepeating == true)
            {
                ...
                else
                {
                    ...
                    ((sp.Children[0] as Grid).Children[0] as Image).Visibility = Visibility.Visible;
                    ((sp.Children[0] as Grid).Children[1] as Image).Visibility = Visibility.Collapsed;
                    tb.Foreground = new SolidColorBrush(Color.FromArgb(255, 30, 30, 35));
                    tb.Text = "Repeat";
        ...
        {
            for (int i = 0; i < StackPanelPivotIndicator.Children.Count; i++)
            {
                var ellipse = StackPanelPivotIndicator.Children[i] as Ellipse;
                var transform = ellipse.RenderTransform as CompositeTransform;
                if (i == MainPivot.SelectedIndex)
                {
                    transform.SetValue(CompositeTransform.ScaleXProperty, 1.5);
                    transform.SetValue(CompositeTransform.ScaleYProperty, 1.5);
                }
                else
                {
                    transform.SetValue(CompositeTransform.ScaleXProperty, 1.0);
                    transform.SetValue(CompositeTransform.ScaleYProperty, 1.0);
                }
            }
        }

        private void musicAlbum_AnimationComplete(object sender, EventArgs e)
        {
            AlbumModel al = new AlbumModel();
            //no song in the phone
            if (MediaService.Instance.CurrentSong == null)
